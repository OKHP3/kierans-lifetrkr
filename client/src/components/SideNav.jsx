...
export default function SideNav({ active, onTabChange }) {
  const { state } = useApp()
  const { theme, setTheme } = useTheme()
  const displayName = state.profile?.displayName
  const pronouns = state.profile?.pronouns
  ...
  return (
    <nav className="side-nav">
      ...
      <div className="side-nav-logo">
        ...
        <div>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 19, fontWeight: 400, color: 'var(--text-primary)', lineHeight: 1.1 }}>
            LifeTrkr
          </div>
          <div style={{ fontSize: 9, color: 'var(--text-ghost)', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 1 }}>
            personal life OS
          </div>
        </div>
      </div>
        ...
        {displayName && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 2px' }}>
            <div style={{
              ...
              fontSize: 11, color: 'var(--accent-amethyst)', flexShrink: 0,
            }}>
            ...
            <div style={{ overflow: 'hidden' }}>
              ...
              {pronouns && (
                <div style={{ fontSize: 10, color: 'var(--text-ghost)' }}>{pronouns}</div>
              )}
            </div>
          </div>
        )}
...
function HomeIcon({ active }) {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2 : 1.6} strokeLinecap="round" strokeLinejoin="round">
