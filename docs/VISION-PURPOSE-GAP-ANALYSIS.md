...
**Prepared:** August 26, 2026; reconciled September 3, 2026
**Scope:** Historical gap snapshot for Kieran's LifeTrkr; current source baseline `v0.1.10`
**Method:** Cross-referenced README, PRD v1-v5, ROADMAP, release records,
HANDOFF, repository-native mirrors, and the July 22 context-thread audit
against actual source behavior. The original findings below are preserved as a
dated snapshot; the current crosswalk is
`docs/VISION-DELIVERY-MATRIX.md`.

> **Current-status notice (September 3, 2026):** This document is not the
> authority for current open items. Several findings below were true on August
> 26 and were subsequently closed in source or evidence work, including the
> dark default, welcome flow, oracle regeneration, privacy route, service
> worker, cat accent, and targeted recurrence/Mercury/oracle checks. Remaining
> owner gates are intentionally not inferred from source inspection. Use the
> current matrix and release baseline for delivery and release claims.
...
- **At the August 26 snapshot, nine in-scope items from the app's own roadmap
  remained undelivered**, most consequentially Google Calendar/Tasks real-account
  activation, live Claude wording, and ownership handoff. This count is
  historical and is not a current release total.
...
- **One doc-hygiene defect was identified at the August 26 snapshot:**
  `docs/ARCHITECTURE.md` was still labeled as the old amendment. It has since
  been replaced with current client-only architecture documentation.
...
## Finding 2: In-scope items not fully delivered (August 26 snapshot)

Verified directly against `src/`, not just doc claims. "In-scope" = appears in
README, PRD v1-v4, or ROADMAP v0.1.x-v1.0.0 (excludes "Post-v1.0 Ideas,"
which the roadmap itself marks not scheduled). This table is retained as the
August 26 evidence snapshot; it is not a current checklist.
...
### Current closure overlay — September 3, 2026

The snapshot above predates the current source and evidence work. The current
disposition is maintained in `docs/VISION-DELIVERY-MATRIX.md`; this compact
overlay prevents the historical “not done” wording from being mistaken for
today's status:

| Snapshot finding | Current disposition |
|---|---|
| Dark default, first-launch flow, oracle regeneration | **Source-confirmed** in the current `v0.1.10` tree |
| Mercury banner, recurrence behavior, horoscope isolation | **Runtime-tested** for the recorded controlled states; date semantics remain a downstream gate |
| Cat accent, privacy route, PWA worker, app identity | **Source-confirmed**; device installation/offline behavior remains owner-controlled |
| Architecture document mismatch | **Closed in documentation**; `docs/ARCHITECTURE.md` now describes the current client-only architecture |
| Google Calendar/Tasks | **Owner-controlled**; local client paths and harness are checked, real-account lifecycle and two-account isolation are not claimed |
| Claude worker wording | **Owner-controlled**; simulated worker boundary and local fallback are checked, live deployment is not claimed |
| Manual accessibility, real storage failure, ownership handoff | **Owner-controlled** and still open in the existing checklists |
| Drag reorder and optional end-of-day review | **Intentionally deferred** from the current release; see the roadmap and deferred-scope task |
...
## Recommendation (historical, not adopted)

**Historical recommendation:** Option B, with Option C's evidence gates folded
in as the gating step before any Google/oracle activation work.
...
Straight sequential roadmap execution (Option A) isn't wrong, just slower to
first real signal, since GCP setup is a hard external dependency that can sit
blocked for days while cheaper wins pile up behind it for no reason. The current
roadmap is the plan of record; this older recommendation does not override it.
...
| `docs/ARCHITECTURE.md` misled a reader at the snapshot date | Closed by the current architecture document; re-check if architecture changes |
...
- [x] Reconcile the historical gap snapshot with current source and release evidence — see `docs/VISION-DELIVERY-MATRIX.md`
- [x] Close the architecture/documentation mismatch, cheap UI items, and targeted behavior checks where current source/evidence supports it
- [ ] Complete the Google, accessibility, storage, device, worker, and ownership gates already linked from the release matrix
