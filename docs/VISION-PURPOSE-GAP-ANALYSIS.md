...
| 7 | PWA offline support (service worker) | README architecture table (implicit), ROADMAP v0.4.0 | **Not built, and correctly disclaimed.** No service worker, no `vite-plugin-pwa` dependency. README and `DEPLOYMENT-CHECKLIST.md` both explicitly say offline isn't a supported claim. | `package.json` dependencies; grep for `serviceWorker`/`VitePWA`: no match |
