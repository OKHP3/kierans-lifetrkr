...
responds to concrete daily-use cases without introducing a second storage model,
a server, Google writes, or a broad review/analytics surface: a user may need to
...
| Evening wrap-up | **Required for this limited increment** | After 6:00 PM in the configured timezone, Today offers an optional read-only closure snapshot. It reports due ritual items, active habit repetitions, and today's local task records, including what remains open. It does not add notes, analytics, a new completion state, or a persisted review record. |
...
- The evening wrap-up is an in-memory projection of existing records. It has no
  persistence key, does not alter completion semantics, and is not shown before
  6:00 PM in the configured timezone.
...
required after this release tree changes. The wrap-up projection also covers
empty, incomplete, timezone-boundary, reload, and account-local states.

## Revisit triggers

Reconsider a broader journal, insight, or analytics surface only when real use
shows the limited closure snapshot is insufficient. Reconsider the broader
OAuth/public-release plan only if the product opens beyond the intended
owner/test-user model.