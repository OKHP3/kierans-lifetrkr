...
| Local routines, habits, and tasks | Daily rituals, recurring habits, active tasks, backlog | Recurrence, categories, descriptions, and Someday terminology | Reducer-backed local records with recurrence fields, category/tag UI, completion state, and `backlog`/`today`/`done` task status | **Source-confirmed** | `src/types.ts`, pages/components, release source checks | Owner browser journey must cover create/edit/complete/delete/reload and malformed or unavailable storage. Any schema, reducer, or persistence-key change expires this claim. |
...
| Drag reorder and end-of-day review | Later polish ideas in PRD-v4 | v0.4 planning items; end-of-day review marked optional | No drag reorder or end-of-day review is in the current source | **Intentionally deferred** | Roadmap/PRD planning records; absence is not presented as a shipped feature | Product-scope decision and an explicit future release plan are required before implementation. |
