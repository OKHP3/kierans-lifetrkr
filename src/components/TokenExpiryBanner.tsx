...
export default function TokenExpiryBanner() {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, background: 'var(--surface-raised)', borderBottom: '0.5px solid var(--border)', padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center' }}>
      ...
      <button onClick={() => setDismissed(true)} style={{ fontSize: 14, color: 'var(--text-ghost)', background: 'none', border: 'none', cursor: 'pointer', marginLeft: 'auto' }}>×</button>
    </div>
  )
}
