...
export default function ThemeToggle() {
  ...
  return (
    <div className="theme-toggle-float" style={{ position: 'fixed', top: 12, right: 'max(12px, calc(50% - 240px + 12px))', background: 'var(--surface-raised)', border: '0.5px solid var(--border)', borderRadius: 20, padding: '2px', gap: 1, zIndex: 150 }}>
      {OPTIONS.map(({ value, label, icon }) => (
        <button key={value} title={label} onClick={() => setTheme(value)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: 16, border: 'none', cursor: 'pointer', background: theme === value ? 'var(--accent-amethyst)' : 'transparent', color: theme === value ? 'var(--bg)' : 'var(--text-ghost)', transition: 'background 0.15s ease, color 0.15s ease' }}>
          {icon}
        </button>
      ))}
    </div>
